...
static int sdSetClock(int freq) {
    ...
    while ((*EMMC_STATUS & (SR_CMD_INHIBIT | SR_DAT_INHIBIT)) && --count)
        sd_delayus(1);
    if (count <= 0) {
        printf("* EMMC ERROR: Set clock: timeout waiting for inhibit flags. "
               "Status %08x.\n",
               *EMMC_STATUS);
...
static void sdParseCID() {
    ...
    printf("- EMMC: SD Card %s %dMb UHS-I %d mfr %d '%s:%s' r%d.%d %d/%d, #%x RCA "
           "%x\n",
