...
void test_sync() {
    ...
    auto check = [&](int j) {
        for (int i = 0; i < OP_MAX_NUM_BLOCKS; i++) {
            int* b =
                reinterpret_cast<int*>(mock.inspect(sblock.num_blocks - 1 - i));
...
void test_parallel(usize num_rounds,
                   usize num_workers,
                   usize delay_ms,
                   usize log_cut) {
...
void test_banker() {
    ...
    for (usize round = 0; round < num_rounds; round++) {
        ...
        if ((child = fork()) == IN_CHILD) {
            ...
            auto duration =
                std::chrono::duration_cast<std::chrono::milliseconds>(end_ts -
                                                                      begin_ts)
                    .count();
